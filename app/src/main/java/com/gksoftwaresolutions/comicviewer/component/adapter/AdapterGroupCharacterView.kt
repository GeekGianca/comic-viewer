...
class AdapterGroupCharacterView(
    private val title: String,
    private val items: PagedListGroup<AdapterCharacterView>
) :
    ...
    override fun bind(viewBinding: ItemGroupLayoutBinding, position: Int) {
        ...
        viewBinding.let {
            ...
            groupieAdapter.setOnItemLongClickListener { item, _ ->
                when (item) {
                    is AdapterCharacterView -> {
                        Toast.makeText(it.root.context, item.title, Toast.LENGTH_LONG).show()
                        true
                    }
                    else -> false
                }
